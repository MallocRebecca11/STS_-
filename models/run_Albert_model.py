...
def model_train_validate_test(train_df, dev_df, test_df, target_dir, 
    ...
    bertmodel = AlbertModel(requires_grad = True, num_labels = num_labels)
...
def model_load_test(test_df, target_dir, test_prediction_dir, test_prediction_name, max_seq_len=64, num_labels=2, batch_size=32):
    bertmodel = AlbertModel(requires_grad = False, num_labels = num_labels)
    tokenizer = bertmodel.tokenizer
    device = torch.device("cuda")
...
if __name__ == "__main__":
    lcqmc_path = "D:/shit/Chinese-sentence-pair-modeling-master/Chinese-sentence-pair-modeling-master/LCQMC/"
    ...
    target_dir = os.path.join(lcqmc_path, "output/Albert/")
    test_prediction_dir = "D://shit"
    model_load_test(test_df, target_dir, test_prediction_dir,"predict")